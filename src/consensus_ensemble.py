...
def run_consensus(
    ...
    if not available_cols:
        raise ValueError(
            f"None of the target score columns {score_cols} found in merged CSV. Available: {list(df.columns)}"
        )
    ...
    df["TCR_Recognition_Propensity_Score"] = compute_borda_scores(
        df_imputed, available_cols, weights
    )
...
def main():
    ...
    parser.add_argument(
        "--score-cols",
        default="sestrav_score,prime_score,predig_score",
    ...
    if args.weights:
        ...
        if len(w_vals) == len(cols):
            ...
            print(
                "[consensus] Warning: Length of weights does not match score-cols. Using uniform weights."
            )
