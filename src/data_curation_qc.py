...
class IEDBDataCurator:
    ...
    def sanitize_input(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Assay - Qualitative Measure"] = (
            df["Assay - Qualitative Measure"].astype(str).str.strip()
        )
    ...
    def resolve_conflicts(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if conflict_ratio > 0.15:
            ...
            raise RuntimeError(
                f"Security/QC Error: Dataset conflict ratio {conflict_ratio:.2f} is too high."
            )
        ...
        if len(consensus) < 500:
            ...
            raise RuntimeError(
                f"Security/QC Error: Post-deduplication yield {len(consensus)} is insufficient."
            )

        logger.info(f"Curated consensus data shape: {consensus.shape}")
        return consensus

    def explicit_negative_mining(
        self, df: pd.DataFrame, binding_threshold: float = 500.0
    ) -> pd.DataFrame:
        """
        ...
        df["Assay - MHC Binding Affinity"] = pd.to_numeric(
            df["Assay - MHC Binding Affinity"], errors="coerce"
        )
        ...
        logger.info(
            f"Explicit negative mining reduced dataset from {len(df)} to {len(mined_df)} records."
        )
        return mined_df
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--check-dataset",
        type=str,
        help="Path to final immunogenicity_dataset.csv to enforce strict QC",
    )
    ...
    if args.check_dataset:
        try:
            ...
            if invalid_mask.any():
                raise ValueError(
                    f"Found {invalid_mask.sum()} peptides with non-canonical amino acids."
                )
