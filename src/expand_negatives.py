...
def find_candidate_negatives(
    data_path, binding_matrix_path, n_per_virus=10, min_presentation_score=0.3, output_dir="results"
):
...
def _write_expansion_report(candidates_df, all_strong, output_dir):
    ...
    for gs in GOLD_STANDARD_NEGATIVES:
        lines.append(f"| {gs['peptide']} | {gs['virus']} | {gs['allele']} | {gs['affinity_nM']} |")
