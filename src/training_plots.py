...
def generate_training_plots(
    data_path, output_dir="results", random_state=42, feature_mode=21, binding_matrix_path=None
):
    ...
    for idx, name in enumerate(models):
        ...
        ax.hist(
            ...
            alpha=0.7,
            color="#55A868",
            label=f"Positive (n={np.sum(yt == 1)})",
            density=True,
        )
        ax.hist(
            ys[yt == 0],
            ...
            alpha=0.7,
            color="#C44E52",
            label=f"Negative (n={np.sum(yt == 0)})",
            density=True,
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "cv_score_distributions.png"), dpi=150, bbox_inches="tight"
    )
    plt.close()
