...
def get_mhcflurry_predictions(peptides, alleles):
    ...
    for i in range(0, len(flat_peps), chunk_size):
        ...
        pred_df = predictor.predict(peptides=pep_chunk, alleles=allele_chunk, verbose=0)
    ...
    pivot_df = df_input_presentation.pivot_table(
        index="peptide", columns="allele", values="presentation_score", aggfunc="mean"
    )
...
def main():
    ...
    data_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../data/immunogenicity_dataset_v4.csv")
    )
    ...
    if matrix_df is not None:
        out_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../models/peptide_binding_matrix_v3.csv")
        )
