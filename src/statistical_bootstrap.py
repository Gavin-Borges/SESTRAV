...
def main():
    ...
    parser.add_argument("--ref-col", required=True, help="Reference model column name (e.g. rf_oof_score)")
    parser.add_argument("--comp-cols", required=True, help="Comma-separated comparison column names (e.g. prime_score,predig_score)")
    parser.add_argument("--label-col", default="label", help="Ground-truth binary label column")
    parser.add_argument("--n-resamples", type=int, default=10000, help="Number of bootstrap resamples (default 10,000)")
    ...
    for col in cols:
        if col not in df.columns:
            ...
            continue
        print(f"[bootstrap] Comparing {args.ref_col} vs {col} (paired bootstrap, N={args.n_resamples})...")
