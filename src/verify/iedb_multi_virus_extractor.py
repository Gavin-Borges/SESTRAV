...
def query_iedb_rest(
    tax_id: int, max_retries: int = 5, backoff: float = 2.0
) -> List[Dict[str, Any]]:
    """
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"Querying IEDB API for TaxID {tax_id} (Attempt {attempt + 1}/{max_retries})..."
            )
            response = requests.get(
                IEDB_URL, params=params, headers=headers, timeout=25, verify=True
            )
...
def clean_and_pool_epitopes(records: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for rec in records:
        ...
        label = (
            1
            if val.startswith("positive") or "positive" in val
            else (0 if "negative" in val else None)
        )
        if label is None:
            continue
...
def load_proteome_peptides(fasta_path: Path, min_len: int = 8, max_len: int = 11) -> List[str]:
    ...
    if not fasta_path.exists():
        logger.warning(
            f"Proteome file {fasta_path} not found. Decoy generation will bypass sequence slicing."
        )
        return []
...
def generate_decoys(
    ...
    if not non_overlapping:
        logger.warning(
            "No safe non-overlapping proteome candidates found. Generating mutated decoys."
        )
        for pos in pos_peptides:
            mutated = list(pos)
...
def process_target(target_name: str, config: Dict[str, Any], data_dir: Path, mock: bool = False):
    ...
    decoy_list = generate_decoys(
        pos_df["peptide"].tolist(), proteome_peptides, target_alleles, decoy_ratio=1.0
    )
