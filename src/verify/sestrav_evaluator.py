...
def evaluate_single_virus(
    ...
    if use_mock or not HAS_PYG or model is None:
        ...
        try:
            ...
            logger.error(f"Failed GNN evaluation for {virus_name}: {e}. Falling back to mock predictions.")
            scores = run_mock_predictions(df)
    ...
    if not df_pos.empty:
        ...
        # Build mutated dfs
        df_anchor = df_pos.copy().drop(columns=["peptide"]).rename(columns={"peptide_anchor_mut": "peptide"})
        df_tcr = df_pos.copy().drop(columns=["peptide"]).rename(columns={"peptide_tcr_mut": "peptide"})
...
def run_evaluation_pipeline(
    ...
    if HAS_PYG and not use_mock:
        try:
            ...
            if model_checkpoint_path and model_checkpoint_path.exists():
                ...
                logger.info("No pre-trained GNN checkpoint found. Evaluating initialized structural GNN baseline.")
        except Exception as e:
    ...
    for virus_name, config in config_matrix.get("viruses", {}).items():
        ...
        if not val_csv.exists():
            logger.info(f"Validation dataset {val_csv} not found. Automatically triggering mock extraction...")
