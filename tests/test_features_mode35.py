...
def _write_binding_matrix(peptides, path):
    ...
    pd.DataFrame([{'peptide': p, **{c: 0.5 for c in allele_cols}} for p in peptides]).to_csv(path, index=False)


def _write_ap_cache(peptides, path, netchop=0.4, tap=0.6):
    pd.DataFrame([{'peptide': p, 'netchop_score': netchop, 'tap_score': tap} for p in peptides]).to_csv(path, index=False)
...
def _write_sim_cache(peptides, path, identity=0.0, exact=False):
    ...
    } for p in peptides]).to_csv(path, index=False)
