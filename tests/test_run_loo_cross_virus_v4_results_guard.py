"""Overwrite-guard tests for scripts/run_loo_cross_virus_v4.py's --output-json
and --output-csv.

Tier-1 instance #5 of the results/ silent-overwrite defect class (see
...
`python scripts/run_loo_cross_virus_v4.py` previously defaulted both flags to
results/loo_cross_virus_v4.json and results/loo_cross_virus_v4.csv and wrote
them with no guard, silently rewriting the tracked artifacts in place.
...
from scripts import run_loo_cross_virus_v4 as loo_v4
...
def test_planned_paths_lists_both_tracked_artifacts(tmp_path):
    output_json = str(tmp_path / "loo_cross_virus_v4.json")
    output_csv = str(tmp_path / "loo_cross_virus_v4.csv")
    paths = loo_v4.planned_loo_paths(output_json, output_csv)
    assert [Path(p).name for p in paths] == ["loo_cross_virus_v4.json", "loo_cross_virus_v4.csv"]
...
def test_guard_passes_when_neither_output_path_exists(tmp_path):
    output_json = str(tmp_path / "loo_cross_virus_v4.json")
    output_csv = str(tmp_path / "loo_cross_virus_v4.csv")
    loo_v4._guard_output_paths(output_json, output_csv, allow_overwrite=False)


def test_guard_refuses_to_clobber_an_existing_json(tmp_path):
    output_json = tmp_path / "loo_cross_virus_v4.json"
    output_csv = tmp_path / "loo_cross_virus_v4.csv"
    output_json.write_text("published", encoding="utf-8")
    with pytest.raises(FileExistsError) as exc:
        loo_v4._guard_output_paths(str(output_json), str(output_csv), allow_overwrite=False)
    assert "loo_cross_virus_v4.json" in str(exc.value)
...
def test_guard_refuses_to_clobber_an_existing_csv(tmp_path):
    ...
    with pytest.raises(FileExistsError) as exc:
        loo_v4._guard_output_paths(str(output_json), str(output_csv), allow_overwrite=False)
    assert "loo_cross_virus_v4.csv" in str(exc.value)
...
def test_guard_names_both_blocking_files_and_the_escape_hatch(tmp_path):
    ...
    assert "loo_cross_virus_v4.json" in message
    assert "loo_cross_virus_v4.csv" in message
...
def test_allow_overwrite_disarms_the_guard(tmp_path):
    ...
    output_json.write_text("published", encoding="utf-8")
    output_csv.write_text("published", encoding="utf-8")
    loo_v4._guard_output_paths(str(output_json), str(output_csv), allow_overwrite=True)
...
def _run_module(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "scripts.run_loo_cross_virus_v4", *args],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
...
def test_cli_actually_threads_allow_overwrite_into_run_loo():
    ...
    """
    source = (REPO_ROOT / "scripts" / "run_loo_cross_virus_v4.py").read_text(encoding="utf-8")
