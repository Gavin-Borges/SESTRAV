...
def test_borda_scores():
    ...
    df = pd.DataFrame({"model_A": [10.0, 20.0, 30.0], "model_B": [0.3, 0.2, 0.1]})
    ...
    scores_weighted = compute_borda_scores(
        df, ["model_A", "model_B"], weights={"model_A": 2.0, "model_B": 1.0}
    )
...
def test_paired_bootstrap():
    ...
    df = pd.DataFrame({"label": labels, "ref": ref_scores, "comp": comp_scores})
