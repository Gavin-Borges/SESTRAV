...
def test_ann_30feature_checkpoint_loads_and_scores():
    ...
    if isinstance(scaler_mean, torch.Tensor):
        assert scaler_mean.dtype == torch.float32, (
            f"Expected float32 scaler_mean, got {scaler_mean.dtype}"
        )
    else:
        assert isinstance(scaler_mean, (list, float)), (
            f"Expected list or float for scaler_mean, got {type(scaler_mean)}"
        )

    if isinstance(scaler_scale, torch.Tensor):
        assert scaler_scale.dtype == torch.float32, (
            f"Expected float32 scaler_scale, got {scaler_scale.dtype}"
        )
    else:
        assert isinstance(scaler_scale, (list, float)), (
            f"Expected list or float for scaler_scale, got {type(scaler_scale)}"
        )
