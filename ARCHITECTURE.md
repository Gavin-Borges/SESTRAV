...
| 4. Immunogenicity scoring | Features + serialized model | RF / XGBoost ensemble; SHAP attribution; isotonic calibration + tuned threshold. **Cross Venn-Abers intervals** (`lower_bound`, `upper_bound`, `interval_width`), on by default, off with `--no-conformal`, emitted only when a calibrator artifact resolves; that artifact is gitignored, so a checkout that has not run `scripts/fit_conformal_calibrator.py` follows `_apply_conformal`: freeze mode raises, while ordinary mode warns before returning point scores only. This was corrected by "sestrav predict dropped conformal intervals without saying so". The earlier split-conformal description stays RETRACTED | `{proteome}_ranked.csv` |
