...
def generate_error_audit(predictions_csv, output_path):
    ...
    def aggregate_errors(group_by_col):
        grouped = df.groupby(group_by_col).agg(
            total=("peptide", "count"), fp=("is_FP", "sum"), fn=("is_FN", "sum")
        )
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Conclusion\n")
        f.write(
            "Use this audit to prioritize feature engineering and allele-aware modeling tracks. Peptides highlighted in the False Positive list repeatedly fool the model and require structural or physicochemical debugging."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to CSV containing predictions and true labels",
    )
    parser.add_argument(
        "--output", type=str, default="results/scoring_error_audit.md", help="Output markdown path"
    )
