...
def ingest_tsnadb(input_path, output_path, schema_path):
    ...
    df_v4["hla_allele"] = df_v4["hla_allele"].str.replace(r"(HLA-[A-Z])(\d)", r"\1*\2", regex=True)
    ...
    write_provenance(
        output_path, sources=[input_path], row_count=len(df_v4), extra={"database": "TSNAdb"}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--schema", default="data/immunogenicity_dataset_v4_schema.json", help="v4 schema path"
    )
