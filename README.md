...
|---|---|---|---|---|---|
| End-to-end workflow (proteome → ranked output) | ✓ | Partial | Partial | Partial (auto-digests a FASTA proteome into peptides and ranks by %Rank; binding/presentation only, no immunogenicity step) | ✓ (neoantigens; authors' own "end-to-end" claim requires pairing with an external variant-calling pipeline upstream) |
| Open source, pip-installable | Open source; `pip install .` from source (not yet on PyPI - `ROADMAP.md`) | Open source (GPL-2.0); not pip-installable - Docker/Singularity or a webserver | Academic/non-commercial license only; not pip-installable - precompiled binary or build-from-source | Academic license; closed source | ✓ |
...
*   **SAST & CI:** Bandit, CodeQL and Semgrep all run on every pull request via GitHub Actions. **Bandit and CodeQL both gate a merge**: each is a required status check on ruleset `16846770`, and CodeQL additionally gates through that ruleset's code-scanning rule. Semgrep runs as two jobs, neither of them a required check: `Semgrep Security Scan (advisory)` scans the `p/python` registry ruleset with `continue-on-error: true` on its scan step, so a finding surfaces as a code-scanning alert and leaves the job green; `Semgrep Custom Rules (blocking)` scans this repo's own three ERROR rules with no `continue-on-error`, so a finding does fail that job visibly, without gating the merge button. Measured 2026-09-13; this bullet previously said only CodeQL gates and that Bandit is not a required check, which contradicted the very document it cites. See `SECURITY.md`'s CI gate map, which is the authoritative list.
...
4. **Immunogenicity Scoring:** Ensemble classification (RF, XGBoost) with SHAP interpretability, a global isotonic calibrator, and a tuned decision threshold. **Stage 4 emits Cross Venn-Abers probability intervals** (`lower_bound`, `upper_bound`, `interval_width`) on the ranked CSV, on by default and turned off with `--no-conformal`. These are Venn-Abers multi-probability bounds, NOT split-conformal sets carrying a fixed 1 - alpha coverage guarantee; the split-conformal description this line carried until 2026-09-07 stays RETRACTED. The calibrator artifact is gitignored, so a checkout that has not run `scripts/fit_conformal_calibrator.py` gets no interval columns. **It is no longer silent about that.** Under the shipped `freeze_mode: true` a run that asks for intervals and cannot resolve a calibrator now RAISES with an actionable message; with freeze mode off it prints a warning naming the three absent columns and both escape hatches (`--no-conformal`, or `--conformal-calibrator` to supply one). See `docs/claims_register.md` D40.
