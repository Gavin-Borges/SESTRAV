...
> **Note (2026-07-04; corrected 2026-07-14, 2026-08-09):** This document reflects early-stage validation results on the 30-feature track (pre-v2 canonical designation). The current canonical production model is the **31-feature RF (`mode_31`)** on the v5 dataset (35,597 active rows). Same-pathogen (within-virus) discrimination is reported as the per-virus within-CV table (mean AUC-ROC 0.751; honest pooled same-pathogen ROC 0.712 on real IEDB negatives, with pooled AUC-PR base-rate-inflated and not a headline); pooled cross-validation AUC-PR for the current corpus is 0.8312 (the previously cited "self-proteome Gate 1 AUC-PR 0.8897" was an error - see below). For current model performance, see `README.md`, `docs/model_evaluation_summary.md`, and `results/per_virus_eval_v5_mode31.csv`. The methodology sections below remain valid historical reference.
>
> **Retraction record (2026-07-11):** The pooled same-pathogen AUC-ROC 0.9368 previously reported in this note (now corrected above) is RETRACTED as decoy-inflated (it only reproduces when synthetic / cross-pathogen decoys, incl. the vaccinia panel, are mixed in as if they were same-pathogen negatives). Current same-pathogen (within-virus) discrimination is reported as the per-virus within-CV table (mean AUC-ROC 0.751; `results/per_virus_eval_v5_mode31.csv`). The honest (decoy-corrected) pooled same-pathogen ROC on real IEDB negatives is 0.712 (pooled AUC-PR is base-rate-inflated and not a headline). **This "honest" is decoy-corrected only, not leakage-corrected: both 0.712 and 0.751 are separately peptide-leakage-exposed (`docs/claims_register.md` D15) and reproduce lower - 0.5989 and 0.6587 - under a peptide-grouped splitter.** See `docs/claims_register.md` D12 (now marked superseded-in-part by D15) and D15.
>
> **Correction (2026-08-09):** "Self-proteome Gate 1 AUC-PR 0.8897", asserted above in prior revisions of this note, was wrong on two counts and is withdrawn, not merely superseded. 0.8897 is the pooled cross-validation `auc_pr` from an earlier (2026-06-26) v5 build, not the 35,597-row corpus this note otherwise describes (which reports 0.8312 for the same metric); and no self-proteome-versus-viral evaluation artifact exists anywhere in this repository - "Gate 1" is a GNN promotion threshold (`src/verify/promote_gnn.py`), not an RF metric. Both the 0.8897 and 0.8312 pooled figures are also leakage-inflated (folds stratified but not grouped by peptide, `docs/claims_register.md` D15). See `docs/claims_register.md` D15/D16.
