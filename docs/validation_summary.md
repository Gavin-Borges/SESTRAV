...
>
> **Retraction record (2026-07-11):** The pooled same-pathogen AUC-ROC 0.9368 previously reported in this note (now corrected above) is RETRACTED as decoy-inflated (it only reproduces when synthetic / cross-pathogen decoys, incl. the vaccinia panel, are mixed in as if they were same-pathogen negatives). Current same-pathogen (within-virus) discrimination is reported as the per-virus within-CV table (mean AUC-ROC 0.751; `results/per_virus_eval_v5_mode31.csv`). The honest pooled same-pathogen ROC on real IEDB negatives is 0.712 (pooled AUC-PR is base-rate-inflated and not a headline). See `docs/claims_register.md`.
>
