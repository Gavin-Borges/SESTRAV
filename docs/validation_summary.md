...
> **Note (2026-07-04; corrected 2026-07-14, 2026-08-09, 2026-08-10):** This document reflects early-stage validation results on the 30-feature track (pre-v2 canonical designation). The current canonical production model is the **31-feature RF (`mode_31`)** on the v5 dataset (35,597 active rows), evaluated under a **peptide-grouped** 5-fold splitter as of 2026-08-10. Same-pathogen (within-virus) discrimination is reported as the per-virus within-CV table (mean AUC-ROC **0.658**; pooled same-pathogen ROC **0.6015** on real IEDB negatives, with pooled AUC-PR base-rate-inflated and not a headline); pooled cross-validation AUC-PR for the current corpus is **0.6055** (`results/pooled_cv_metrics_mode31.csv`, row `mode31_pooled_auc_pr`, `kind = pooled_single_pass`; the previously cited "self-proteome Gate 1 AUC-PR 0.8897" was an error - see below). The fold-mean of the same five per-fold AUC-PR values - as opposed to the pooled computation above - is a distinct **0.6058** (`models/v5/training_results_mode31.csv`, `rf_cv_mean`), the figure cited elsewhere in this repository against the mode-33/35 antigen-processing ablation ladder. Cite each against the artifact that holds it: `training_results_mode31.csv` contains no `0.6055`. For current model performance, see `README.md`, `docs/model_evaluation_summary.md`, and `results/per_virus_eval_v5_mode31.csv`. The methodology sections below remain valid historical reference.
>
