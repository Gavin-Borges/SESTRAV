...
- **Model file:** `models/rf_33feature_integrated.joblib` - **not distributed with this repository.** `models/*.joblib` is gitignored, so a fresh clone contains no model binary (confirm the live rule with `git check-ignore -v models/rf_33feature_integrated.joblib`). **Unlike `rf_31feature_integrated.md`, this card carries no runnable rebuild command, so a reader cannot reproduce this binary from it.** The Training script entry under [Provenance](#provenance) records only the two distinguishing flags, `--feature-mode 33 --sample-weights`, and omits `--data`, which the tracked script marks `required=True`; run as written it exits 2 with `error: the following arguments are required: --data, --model-dir` and trains nothing. That entry is a **partial record of a v3-generation run, not an invocation that ever ran as written**: `--data` has carried `required=True` in every tracked revision of the script, without exception, back to its first (`69e0e5c`, 2026-04-26), so no **tracked** era exists in which the recorded line was complete. ("Tracked" is doing real work in that sentence and is not hedging: the H3 flag above entertains an unpreserved earlier script version, and this claim deliberately does not reach past what history holds.) Do not "repair" it by supplying the missing arguments - that would fabricate a full invocation no record supports, and the v3 corpus it would need is not the tracked v5 one. For a command that does run from a clean clone see `rf_31feature_integrated.md` under Provenance, noting it trains the 31-feature canonical model and **not** this configuration.
- **Provenance:** `models/model_artifact_checksums.json` - records the SHA-256 digest and byte size of `rf_33feature_integrated.joblib`, so a binary obtained by other means can be checked against the reference build. See [Provenance](#provenance) below for the full record.
...
- Artifact checksum: `models/model_artifact_checksums.json` (SHA-256 + byte size for `rf_33feature_integrated.joblib`; helpers in `src/artifact_integrity.py`)
- Training script: `src/train_classifier.py --feature-mode 33 --sample-weights`
- Training artifacts: `models/training_results.csv` **at commit `6a51995`**, `models/feature_importances.csv`. The SHA anchor is load-bearing and is the repair this card's own S1 note (above) prescribed but did not apply until 2026-08-31: the *live* `models/training_results.csv` is now a v5 mode-31 run and binds none of the mode-33 v3 figures in this card. At `6a51995` it holds RF AUC-PR 0.8399148 / AUC-ROC 0.6727537 and XGB 0.8234847 / 0.6392901, so the four AUC cells reproduce exactly there. The four ISSR cells reproduce from nothing at that commit or any other and remain withdrawn per that note.
