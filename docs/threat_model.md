...
| T4 | **Tampering** - compromised dependency | Hash-pinned lockfiles (`--require-hashes`); `dependency-review.yml` flags vulnerable deps in PRs (advisory - it fails its own job but is not a required status check, so it does not gate the merge button); Dependabot + `pip-audit` monitor; SLA in `SECURITY.md` | `.github/workflows/dependency-review.yml`; `.github/dependabot.yml`; `environments/requirements.lock` |
| T5 | **Information disclosure** - secret/PII leak | No credential store; `pii_scan.yml` has two halves: a diff-scoped pre-merge gate over added lines, and a content-scoped weekly sweep that checks tracked CONTENT for workstation absolute paths and for AI-footprint files. The sweep fetches `refs/heads/main`, `refs/heads/release/*` and `refs/tags/*`, so it reaches main, the release branches and every tag, not every published branch; its AI-footprint list mirrors the AI-assistant, session-state and prompt entries of `BLOCKED_PATTERNS` in `scripts/hooks/pre-commit` and its exemptions mirror that hook's `PATH_SCAN_EXEMPT`, but it does not carry the hook's credential-file entries (`.key`, `.pem`, `.p12`, `.pfx`, `.env`). The sweep is a detection control: it prevents and detects the next leak, it does not un-publish one that is already public; `.gitignore` catch-alls for key/cert/db files | `.github/workflows/pii_scan.yml`; `scripts/hooks/pre-commit`; `SECURITY.md` (PII/PHI section) |
