...
- **Statement coverage (OpenSSF Silver `test_statement_coverage80`):** Coverage is
  ...
  - *Whole-repo floor* - a regression floor across the entire tree
    (`pyproject.toml`), gated at `fail_under=35`, currently **49.36%** (branch-inclusive,
    re-measured 2026-08-16 at `a336360`) - comfortably above the floor. Supersedes both
    the 47.88% (2026-08-08) and the 49.71% (2026-08-15) readings, and before those the
    34.37% / "a hair under the floor" (measured 2026-06-22). The figure drifts with
    ordinary development, so re-measure rather than restate it.
...
| `tests/test_graph_builder.py:48` (2026-06-18 scan; now line 67) | `torch.save(dist, tmp_path / name)` - test fixture writing a known tensor; the name is now built by `structural_cache_filename` | `# nosec B614` added 2026-06-18 |
| `tests/test_graph_builder.py:80` (2026-09-04; now line 90) | `torch.save(` - second test fixture, added with the allele-keyed structural cache, writing a known tensor. It now writes the LEGACY `PEP_dist.pt` name on purpose, so the test fails if the reader ever accepts a peptide-only key again | `# nosec B614` added 2026-09-04 |
| `tests/test_graph_builder.py`, in `test_build_spatial_adj_is_allele_specific`, `test_report_structural_cache_coverage_counts_partial_hits` and `test_report_structural_cache_coverage_is_quiet_prefix_on_full_hit` (2026-09-04) | `torch.save(` - three further fixtures, each writing a known tensor. Cited by test name rather than line number on purpose: the two rows above have been re-anchored twice already, and the symbol does not move | `# nosec B614` added 2026-09-04 |
