...
*   **SESTRAV Status:** ✅ **PASSING**
*   **Evidence:**
    *   **Lockfiles with Hashes:** Dependency lists are compiled and locked with SHA-256 verification hashes inside [requirements.txt](../requirements.txt) to prevent dependency hijacking. For manifests that have a `.in` source the compiler is `tools/update_dependencies.py`, a wrapper around `uv pip compile --generate-hashes`, and the exact argv is recorded in each such manifest's own header. The tier-4 hand-maintained locks in [CONTRIBUTING.md](../CONTRIBUTING.md) have no `.in` source and are curated with `pip download` + `pip hash`. (Corrected 2026-09-20: this named `pip-compile`, accurate when written on 2026-06-13. Re-corrected the same day: a first replacement generalised the tool-compiled path to every manifest and dated the uv move to a single day; it was staggered per manifest between 2026-07-28 and 2026-08-22.)
