...
*Status: Active draft. v5 RF complete (AUC-PR 0.8897, AUC-ROC 0.9760; 26,935 active rows). GNN v5 retraining pending (ESM-2 t12 cache rebuild required for 25,386 unique v5 peptides). External benchmark Table 3 complete (§3.3; v3 baseline). Pending: v5 number update throughout (§3.1, §3.4, §3.5), Gate 1 v5 GNN evaluation, Zenodo DOI (§5).*
...
**Gate 1 resolution.** Three architectural iterations on v4 - including the variable-length graph fix originally hypothesized as the primary bottleneck - all plateau at mean-fold AUC-PR in the range 0.72-0.73. On identical v4 OOF folds the honest like-for-like comparison is GNN v2.3 (AUC-PR 0.7281) vs v4 RF mode-31 (AUC-PR 0.7635); the GNN trails by 0.035. The padding fix's marginal gain confirms the GNN is not gated by an implementation defect but is at its representational ceiling on v4 data. Expansion of the training dataset to v5 (26,935 active rows, incorporating curated virus-specific hard negatives per virus) raises the v5 RF mode-31 to AUC-PR **0.8897** - clearing the Gate 1 threshold of 0.85 and establishing that the threshold is reachable with sufficient negative-class realism. The GNN v2.3 on v4 data (0.7281) now trails the v5 RF by 0.1616. GNN retraining on v5 is in progress (pending ESM-2 t12 cache rebuild for all 25,386 unique v5 peptides); this section will be updated with v5 GNN results. Accordingly, **the v5 canonical Random Forest (mode-31, AUC-PR 0.8897) is designated the production immunogenicity scorer**, and the GINEConv+ESM-2 GNN is reported as a research and ensemble-candidate component pending v5 evaluation.
...
| Variant | Pooling | ESM-2 | Mean-fold AUC-PR | vs v5 RF (0.8897) | vs v2.3 (0.7281) | Decision |
|---|---|---|---|---|---|---|
| GNN v2.1 (baseline) | mean | t6, 320-dim | 0.7241 +/- 0.0087 | -0.1656 | - | Research |
| GNN v2.2 | mean | t12, 480-dim | 0.7160 | -0.1737 | -0.012 | No gain |
| GNN v2.3 (var-length fix) | mean | t12, 480-dim | 0.7281 +/- 0.0070 | -0.1616 | +0.012 | Research |
| **GNN v2.4 Lever 1** | **attention** | **t12, 480-dim** | **0.7228 +/- 0.0070** | **-0.1669** | **-0.005** | **Regression** |
