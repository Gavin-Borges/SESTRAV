...
| Variant | Pooling | ESM-2 | OOF AUC-PR | vs RF (0.7635) | vs v2.3 (0.7263) | Decision |
|---|---|---|---|---|---|---|
| GNN v2.1 (baseline) | mean | t6, 320-dim | 0.7241 ± 0.0087 | −0.039 | — | Research |
| GNN v2.2 | mean | t12, 480-dim | 0.7160 | −0.048 | −0.008 | ❌ No gain |
| GNN v2.3 (var-length fix) | mean | t12, 480-dim | 0.7263 | −0.037 | +0.010 | Research |
| **GNN v2.4 Lever 1** | **attention** | **t12, 480-dim** | **0.7228 ± 0.0070** | **−0.041** | **−0.004** | **❌ Regression** |
...
5. **GNN gate status.** GraphPredictor v1 fails Gates 1 and 5 (v4 AUC-PR 0.613, sensitivity 0.724). GraphPredictorV2 (GINEConv + ESM-2) clears Gates 2–5 but plateaus at OOF AUC-PR ≈ 0.72–0.73 across v2.1–v2.4, below the structurally unreachable Gate 1 threshold of 0.85 (0.09 above the v4 RF ceiling of 0.7635; §3.6). v2.4 Lever 1 (attention pooling, t12 480-dim) regressed slightly vs v2.3 (0.7228 vs 0.7263); the attention gate does not unlock the representation at current data scale. The production scorer is the v4 canonical Random Forest (mode-31); the GNN is a research/ensemble component, not the gated production model.
...
3. **GNN v2.4 (completed, negative result).** v2.4 Lever 1 (attention pooling, t12 480-dim, mode-31, identical OOF folds) achieved mean AUC-PR 0.7228 ± 0.0070 — a regression of −0.004 vs v2.3 mean pooling and −0.041 below the RF bar (0.7635). The attention gate did not unlock the representation; Lever 2 (t30 640-dim) was not executed per protocol. The negative result is documented honestly in §3.6. Future work: larger training datasets, ESMFold spatial contact edges (Lever 3), or multi-graph ensembling may be needed before the GNN surpasses the RF on v4 data.
