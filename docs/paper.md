...
| Variant | Pooling | ESM-2 | Mean-fold AUC-PR | vs RF (0.7635) | vs v2.3 (0.7281) | Decision |
|---|---|---|---|---|---|---|
| GNN v2.1 (baseline) | mean | t6, 320-dim | 0.7241 ± 0.0087 | −0.039 | - | Research |
| GNN v2.2 | mean | t12, 480-dim | 0.7160 | −0.048 | −0.012 | ❌ No gain |
| GNN v2.3 (var-length fix) | mean | t12, 480-dim | 0.7281 ± 0.0070 | −0.035 | +0.012 | Research |
| **GNN v2.4 Lever 1** | **attention** | **t12, 480-dim** | **0.7228 ± 0.0070** | **−0.041** | **−0.005** | **❌ Regression** |
...
**Interpretation.** Attention pooling with a single linear gate regresses by −0.005 AUC-PR relative to v2.3 mean pooling, does not clear the RF bar (0.7635), and triggers the per-protocol decision rule: Lever 2 (t30 ESM-2, 640-dim) is not warranted because attention pooling did not first unlock the representation - the same pattern as v2.2 showing t6→t12 capacity did not help. The early-stopping variance across folds (best epoch range 6–35) and fold 1's higher AUC-PR (0.7363 vs 0.72 elsewhere) suggest the attention gate overfits on certain fold partitions without generalising. **The GNN v2.4 experiment is a publishable negative result.** The production scorer remains the v4 RF mode-31. Lever 2 (t30) and Lever 3 (ESMFold spatial edges) are deferred to future work; they remain valid architectural hypotheses but should be revisited after additional training data or a structural inductive bias more expressive than the current chain-only topology.
...
5. **GNN gate status.** GraphPredictor v1 fails Gates 1 and 5 (v4 AUC-PR 0.613, sensitivity 0.724). GraphPredictorV2 (GINEConv + ESM-2) clears Gates 2–5 but plateaus at mean-fold AUC-PR ≈ 0.72–0.73 across v2.1–v2.4, below the structurally unreachable Gate 1 threshold of 0.85 (0.09 above the v4 RF ceiling of 0.7635; §3.6). v2.4 Lever 1 (attention pooling, t12 480-dim) regressed slightly vs v2.3 (0.7228 vs 0.7281); the attention gate does not unlock the representation at current data scale. The production scorer is the v4 canonical Random Forest (mode-31); the GNN is a research/ensemble component, not the gated production model.
...
2. **Beyond hard decoys - within-virus negative realism.** The 5,000 central-tolerance self-peptides already integrated in v4 decoupled binding from immunogenicity in the negative class (restoring binding features to top-10 RF importance; §3.2), but yielded mode-31 AUC-PR 0.7635 - not the >0.880 originally projected. Removing the binding shortcut exposes a genuinely harder task rather than inflating raw discrimination, and the LOO analysis (§3.4.2) shows the residual gap is within-virus discrimination. The next lever is negative-set realism *within* each virus - experimentally-confirmed non-immunogenic viral peptides rather than self-proteome decoys - to give the model signal that separates immunogenic from non-immunogenic peptides of the same pathogen.
3. **GNN v2.4 (completed, negative result).** v2.4 Lever 1 (attention pooling, t12 480-dim, mode-31, identical OOF folds) achieved mean AUC-PR 0.7228 ± 0.0070 - a regression of −0.005 vs v2.3 mean pooling and −0.041 below the RF bar (0.7635). The attention gate did not unlock the representation; Lever 2 (t30 640-dim) was not executed per protocol. The negative result is documented honestly in §3.6. Future work: larger training datasets, ESMFold spatial contact edges (Lever 3), or multi-graph ensembling may be needed before the GNN surpasses the RF on v4 data.
...
- **Docker image:** Planned for a future release (GitHub Container Registry). For this release, `pip install sestrav`, the pinned `config.yaml` model versions, and the Zenodo dataset deposition together provide full reproducibility.
