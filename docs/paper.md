...
and results/loo_cross_virus_v5_clean.csv; the decomposition table is archived for the
submission build and pending promotion to a committed results/ CSV for integrity binding.)
...
reproducible via the frozen Tier A scores; the summary is archived for the submission
build and pending promotion to a committed results/ CSV so every reported statistic is
integrity-bound.)
