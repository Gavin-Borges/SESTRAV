...
stratified but not grouped by peptide, so 71.0% of held-out rows share their
...
on the remaining folds. **This is not a leakage-free guarantee.** The splitter
stratifies by class and length but does not group by peptide, and the dataset is
deduplicated on `(peptide, hla_allele)` rather than on peptide alone; because every
mode-31/mode-30 feature is a pure function of the peptide string, 71.0% of held-out
rows in the certified cross-validation share their exact peptide with the training
fold and are feature-identical to a training row. Held at the production
configuration, this inflates pooled AUC-PR by +37.0%, the per-virus mean by
+14.0%, and the pooled same-pathogen figure by +19.0% (`docs/claims_register.md`
D15). The production model retrained on the complete active pool after
cross-validation completed is used for inference and deployment only; it is not
...
fully scored on the shared set from its public release checkpoint. SESTRAV's arm
reflects out-of-fold predictions from the ungrouped splitter described above, so the
comparison is not conservative for SESTRAV as previously claimed here: the leakage
runs toward SESTRAV, making its arm optimistic rather than handicapped relative to
the fully-scored external tools. The head-to-head is reported in Section 3.5.
...
version of this section, that this gap indicates cross-viral training provides
complementary discriminatory signal.** Both the pooled figure and the per-virus
within-CV mean are computed under the same ungrouped, peptide-leaking splitter
described in Section 2.4, but the leakage differs in magnitude between them
(pooled +37.0% vs. per-virus mean +14.0%, `docs/claims_register.md` D15); the gap
this paragraph previously read as a biological finding is substantially explained
by that differential inflation, not by a validated cross-viral signal. HPV and HCV
are the lowest-performing viruses in within-CV evaluation; both
...
arm is optimistic rather than conservative: 71.0% of held-out rows in the certified
