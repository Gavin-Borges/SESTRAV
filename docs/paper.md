...
SESTRAV addresses these gaps through workflow integration rather than model novelty alone. The six-stage architecture - (1) proteome-scale peptide generation, (2) MHC binding prediction, (3) TCR-contact physicochemical feature extraction, (4) antigen processing scoring, (5) immunogenicity inference, (6) freeze-mode governed output - is the primary contribution; each stage is individually replaceable without breaking the pipeline DAG. Reproducibility governance is a first-class design goal: each training run records the SHA-256 digest and byte size of every artifact it writes into `models/model_artifact_checksums.json` (via `src/artifact_integrity.py`); the training corpus is pinned by a build-time provenance sidecar (`data/immunogenicity_dataset_v5_provenance.json`, whose `output_checksum_sha256` field verifies against the tracked `data/immunogenicity_dataset_v5.csv`); and the MHCflurry model version (`mhcflurry_model_version: 2.2.1`) and canonical feature mode (`feature_mode: 31`) are pinned in `config.yaml`. No publicly available tool executes all six stages in a single reproducible command with OpenSSF-compliant supply-chain security.
...
*v4 runs are unweighted 5-fold OOF (`src/train_classifier.py --feature-mode {31,33,35}`). The v4 mode-35 metrics and OOF predictions are committed at `models/training_results_mode35.csv` and `models/rf_oof_predictions_mode35.csv` (last written at commit 211bf34, 2026-06-22), so the mode-35 row is verifiable from a clean clone. The mode-31 pair is not: `models/training_results_mode31.csv` and `models/rf_oof_predictions_mode31.csv` were overwritten in place by the v5 retrain (commit e6aafe2 onward) and now hold v5 content, so the v4 mode-31 figure (regenerated 2026-06-22: AUC-PR 0.7635 +/- 0.0093, AUC-ROC 0.7808) is a historical baseline that is **not** reproducible from a clean clone - it is recoverable only from git history, via `git show 211bf34:models/training_results_mode31.csv`. The v5 RF mode-31 figures previously quoted here (AUC-PR 0.7678 within-virus, AUC-ROC 0.9368; commit e6aafe2) are retracted as decoy-inflated per `docs/claims_register.md` D12; the Gate 1 self-proteome AUC-PR 0.8897 is unaffected by that retraction.*
