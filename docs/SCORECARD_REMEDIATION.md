...
| 6 | Pinned-Dependencies (score 9/10) | Medium | ⬜ The `scorecard-action` upgrade changed which scanner version runs, not what it measures, so it does not remediate this check. It still scores 8/10 live (verified 2026-08-23 at commit 36e3d8d, Scorecard v5.5.0: reason "dependency not pinned by hash detected -- score normalized to 8"; 26 of 34 pipCommand dependencies pinned). The warned sites are the two `pip install` steps in each of `Dockerfile`, `Dockerfile.api` and `Dockerfile.demo`, plus two in `.github/workflows/release.yml` (the release-check venv install, and the PyPI availability poll). Only `.github/workflows/release.yml` is still byte-identical at HEAD to the measured commit. `Dockerfile` changed 2026-09-06, and `Dockerfile.api` and `Dockerfile.demo` changed 2026-09-07: all three now bootstrap pip from `environments/requirements-pip-bootstrap.txt` with `--require-hashes`. The api and demo change was a SECURITY fix, not a score fix - both were pinned to pip 26.1.2, inside the affected range for GHSA-qwm4-qh6w-59xr (first patched 26.2.0), and the exposure was invisible on the Security tab because alerts #35-#38 were dismissed as false positives. That dismissal was half right: it holds for the `.[api]`/`.[demo]` local-path half of each RUN line and not for the `pip==` half, which is a remote artifact and is hash-pinnable. Scorecard emits one result per unpinned pip command but anchors each to the whole RUN span, so a single comment landed on a pair where it was true of only one member. Its pip bootstrap, previously `pip install --user "pip==26.1.2"`, now installs `environments/requirements-pip-bootstrap.txt` with `--require-hashes`. The 8/10 above PREDATES that edit and is deliberately not restated: Scorecard has not run against the new content, so any improved figure would be unmeasured, and the next scheduled run is what will re-measure. The OTHER `Dockerfile` pip site, `pip install --user --no-deps .`, is unchanged and still expected to be warned: `isUnpinnedPipInstall` in `checks/raw/shell_download_validate.go` treats a bare local `.` as an additional argument, and accepts a local source only as `-e .` alongside `--no-deps`, or as a prebuilt `.whl`. That reading is of ossf/scorecard `main` fetched 2026-09-06, NOT the v5.5.0 revision behind the 8/10, so re-check it against the version the next run reports before relying on it. Line numbers are deliberately omitted here; Scorecard reports them per run and they rot |
...
We have successfully regenerated `requirements.txt` from `requirements.in` using `pip-compile`, the compiler for **that file** until it moved to `uv pip compile` (via `tools/update_dependencies.py`) on 2026-07-28. (Scoped 2026-09-20: this sentence read "this repo's compiler until it moved", which states a repo-wide move on a single date. The move was staggered per manifest, first on 2026-07-28 and last on 2026-08-22; re-derived with `git log --reverse -S'autogenerated by uv'` over each manifest, which returns three distinct dates. The same correction is already recorded in `docs/openssf_best_practices_readiness.md` and `docs/security_compliance.md`, which this sentence contradicted.)
